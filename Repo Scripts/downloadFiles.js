...
function saveProblem() {
        var scriptTxt = getScript();
        var descTxt = getDescription();
        var fnName = getFnName();
        saveScript(scriptTxt, fnName);
        saveDescription(descTxt, fnName);
}

function getScript() {
        var rawTxt = document.getElementsByClassName("CodeMirror-lines")[0].innerText;
        var rawArr = rawTxt.split("\n");
        var scriptArr = [];
        for (var i = 1; i < rawArr.length; i = i + 2) {
                scriptArr.push(rawArr[i]);
        }
        return scriptArr.join("\n")
}

function getDescription() {
        var d = document.getElementsByClassName("markdown")[0];
        return toMarkdown(d.innerHTML, { gfm: true });
}

function getFnName() {
        return document.getElementsByClassName("header-navigation--title")[0].innerText;
}

function saveScript(scriptTxt, fnName) {
    saveTextFile(scriptTxt, fnName + ".js");
}

function saveDescription(descTxt, fnName) {
    saveTextFile(descTxt, fnName + ".md");
}

function saveTextFile(saveStr, fileName) {
    var blob = new Blob([saveStr],{
 ...
 */
            ...
            function bfsOrder(node) {
                var inqueue = [node], outqueue = [], elem, children, i;
 ...
 */
            ...
            toMarkdown = function(input, options) {
                ...
                var clone = htmlToDom(input).body, nodes = bfsOrder(clone), output;
